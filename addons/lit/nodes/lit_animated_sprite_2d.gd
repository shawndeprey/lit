@tool
@icon("res://addons/lit/icons/lit_animated_sprite_2d.svg")
extends AnimatedSprite2D
class_name LitAnimatedSprite2D

## An AnimatedSprite2D that ships pre-wired with the lit_receiver ShaderMaterial and an
## empty SpriteFrames, so it animates exactly like the stock node and is lit by Lit
## with no manual setup: the LitSprite2D counterpart for frame animation. This is the
## from-scratch path; "Make Selected Nodes Lit" is the batch path for existing art. It
## is just a shortcut, equivalent to assigning the receiver material to a plain
## AnimatedSprite2D by hand.
##
## Normal and specular maps come from the frames themselves: Lit reads them off a
## CanvasTexture, exactly as it does for LitSprite2D, and re-checks the specular slot
## every time the frame changes. Two frame shapes work:
##  - a CanvasTexture per frame, with diffuse / normal / specular set on each; or
##  - for sprite sheets, ONE CanvasTexture carrying the diffuse, normal and specular
##    sheets, with an AtlasTexture per frame whose `atlas` is that CanvasTexture. The
##    SpriteFrames panel's "Create Frames from Sprite Sheet" builds exactly this when
##    pointed at a saved CanvasTexture resource.
## A CanvasTexture whose own slots are AtlasTextures does NOT work: Godot samples the
## whole sheet instead of the region (on Sprite2D too), so avoid that shape.
##
## Exposes the receiver shader's per-instance parameters as @exports that proxy to
## this node's material, so every LitAnimatedSprite2D can be tuned and masked
## independently. At runtime, receivers with identical material content share one
## pooled material (batching + one uniform set per distinct configuration); proxy
## edits re-key the node to the matching pool entry, and make_material_unique()
## detaches it for raw set_shader_parameter writes.
...
# SpriteFrames panel), so the specular flag follows the edited frame set.
var _watched_frames: SpriteFrames = null

# The CanvasTexture behind the CURRENT frame, watched for specular-slot changes so
# has_specular_map re-evaluates live when the user assigns or clears a specular map.
...
func _init() -> void:
	...
	if material == null:
		...
		# Fast by default: a fresh LitAnimatedSprite2D has no owned occluders.
	...
	# (or a saved scene's) replaces it.
...
func _lit_ready() -> void:
	...
	# Keep has_specular_map in sync with the frame on screen, so the Blinn-Phong path
	# picks the half-vector specular only when the current frame actually carries a
	# specular map (it blows out without one). The frame can change through the
	# animation itself, an animation switch, a SpriteFrames swap, or a frame edit in
	# the SpriteFrames panel; each lands in _on_frame_changed. Connect first, then
	# evaluate once for the frames the scene deserializer already set.
	...
	if not animation_changed.is_connected(_on_frame_changed):
...
# registry's variant walk re-points their material on activity changes. Frame
# animation never needs it: the specular flag is signal-driven, not polled.
func _update_process_state() -> void:
	set_process(Engine.is_editor_hint() or not _self_occluders.is_empty()
			or shadow_ignore_mask != 0)


# Re-point the frame-edit subscription at the current SpriteFrames (it emits `changed`
# when frames are added, replaced or removed), then re-resolve the current frame.
func _on_sprite_frames_changed() -> void:
	if _watched_frames != null and is_instance_valid(_watched_frames):
		if _watched_frames.changed.is_connected(_on_frame_changed):
			_watched_frames.changed.disconnect(_on_frame_changed)
	_watched_frames = sprite_frames
	if _watched_frames != null and not _watched_frames.changed.is_connected(_on_frame_changed):
		_watched_frames.changed.connect(_on_frame_changed)
	_on_frame_changed()


# Re-point the specular-slot subscription at the current frame's CanvasTexture, then
# refresh the flag. Frames sharing one CanvasTexture sheet (the sprite-sheet shape)
# resolve to the same texture every frame, so the subscription never churns there.
func _on_frame_changed() -> void:
	var ct := _current_canvas_texture()
	if ct != _watched_texture:
		if _watched_texture != null and is_instance_valid(_watched_texture) \
				and _watched_texture.changed.is_connected(_update_specular_flag):
			_watched_texture.changed.disconnect(_update_specular_flag)
		_watched_texture = ct
		if ct != null and not ct.changed.is_connected(_update_specular_flag):
			ct.changed.connect(_update_specular_flag)
	_update_specular_flag()


# The CanvasTexture the current frame draws with, looked up through AtlasTexture
# regions (a sheet-backed frame is an AtlasTexture over the CanvasTexture sheet).
# Null when there is no such frame or it isn't CanvasTexture-backed.
func _current_canvas_texture() -> CanvasTexture:
	var frames := sprite_frames
	if frames == null or not frames.has_animation(animation):
		return null
	if frame < 0 or frame >= frames.get_frame_count(animation):
		return null
	var tex: Texture2D = frames.get_frame_texture(animation, frame)
	while tex is AtlasTexture:
		tex = (tex as AtlasTexture).atlas
	return tex as CanvasTexture

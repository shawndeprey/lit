...
`addons/lit/editor/lit_migrations.gd`, and `Test/gate_migration_schema.gd` fails
whenever a class's live schema differs from the locked baseline advanced through the
`MIGRATIONS` list. If a PR renames, removes, retypes, re-defaults, or adds a stored
property on any Lit node - or adds a new Lit node class - the gate stays red until the
PR appends a `MIGRATIONS` entry (or, for a brand-new class, a schema entry) describing
the change from the previous release version; never edit `BASELINE_SCHEMA` in place.
Migration entries are chained per node oldest-to-newest by the "Update Project to Lit"
tool; value-transforming `apply` functions receive the node plus its as-saved property
